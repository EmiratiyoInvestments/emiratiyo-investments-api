...
public class TCSNotifyMeController {
    ...
    public ResponseEntity<ApiResponse<String>> notifyMeInfo() {
        return ResponseEntity.ok(ApiResponse.success("Use POST with JSON body to submit notify-me signup."));
    }
    ...
    public ResponseEntity<ApiResponse<String>> notifyMe(@Valid @RequestBody TCSNotifyMeRequest request) {
        ...
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success("Thanks! We'll notify you when new courses launch."));
        } catch (Exception e) {
